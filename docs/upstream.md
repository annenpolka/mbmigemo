...
本プロジェクトのライセンスは2026-09-11にMITと決定し、ルートの`LICENSE`に本文を置く。移植元のC/Migemoと変換表がMITであり、コンパイルで混入するMoonBit標準ライブラリのApache-2.0部分は自身のライセンスとNOTICEを保持する構成と両立する。実用辞書はGPL-2.0-or-laterの別取得データで、パッケージへ同梱しないためコードへ伝播しない。上流のライセンスを本プロジェクトへ自動で適用したことにはしない。
