...
`src/dictionary/` は長さ・LOUDSの到達可能性と子の順序・対応区画と値ノードの範囲・余分な末尾バイトを初期化時に検証する。検証後は文字の辺、子の範囲、親、候補対応を平坦な配列で所有し、元のBytesは保持しない。問い合わせごとに辞書全体を再構築せず、読みの前方一致の部分木だけを列挙する。配列への展開によるメモリ増加は実用辞書で測定する。

jsmigemo builderの正規の空辞書は54バイトで、両trieの辺はdummy/rootの二つ、LOUDSは先頭の1ビットだけになる。この特殊形も検証して受け入れる。空の読み、空の候補、NUL、値の単独サロゲートは形式上表現できるため、構造の破損とは扱わない。テスト用TSVの入力制限とは別の契約である。
